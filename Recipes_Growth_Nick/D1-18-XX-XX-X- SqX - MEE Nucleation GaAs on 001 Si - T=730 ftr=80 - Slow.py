...
###########################
rate_ga = 0.17  # A/s - Will calculate cell temp from this based on latest RHEED/BFM calibrations
p_as_gaas = 2.5E-6  # Torr - Will calculate valve opening from this based on latest BFM calibrations
nuc_rate_ga = 0.33  # A/s - Will calculate cell temp from this based on latest RHEED/BFM calibrations
nuc_p_as_gaas = 5E-6  # Torr - Will calculate valve opening from this based on latest BFM calibrations
###########################
...
if __name__ == '__main__':
    ...
    with MBERecipe(virtual_server=run_virtual_server) as mbe:
        ...
        T_nuc_Ga = calib_Ga.calc_setpoint_gr(nuc_rate_ga)
        ...
        T_Des_GaAs_Manip = 730  # Desired manip temperature (pyro is broken)
        nuc_as_valve_gaas = calib_As.calc_setpoint(nuc_p_as_gaas)
        as_valve_gaas = calib_As.calc_setpoint(p_as_gaas)
        t_anneal = 30 * 60  # 30 minutes
        t_nuc_gaas = 10 * 60  # mee nucleation
        t_growth_gaas = 100 * 60  # 50 minutes
        ...
        mbe.set_param("Ga.PV.TSP", T_nuc_Ga)
