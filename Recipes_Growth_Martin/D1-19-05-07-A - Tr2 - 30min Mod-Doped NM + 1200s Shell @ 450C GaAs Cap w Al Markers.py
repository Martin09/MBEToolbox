...
# 18-10-16-B: Repeat this recipe from 10-12-D due to possible stuck Ga shutter
# 18-11-04-A: Decreased shell growth time to 600s from 1000s
# 18-11-07-C: Reverted to old calibraiton files (don't trust new BFM before we do RHEED)
# 19-04-04-E: Updated cell temperatures for Al, Ga and Manip (since we are growing on Tr holder).
# 19-04-12-B: Doubled shell growth time from 600s to 1200s
# 19-04-16-C: Repeat of 12-B since Ga shutter wasn't working
# 19-05-07-A: Manually opening/closing Al shutter to make sure this sample works
###########################
thickness_almarkers = 5
thickness_markerspacer = 10
thickness_dop = 10  # 10nm thick doped layer
thickness_barrier = 10  # 10nm thick barrier
t_growth_shell = 1200  # seconds
...
rate_al_shell = 0.5  # A/s - Will calculate cell temp from this based on latest RHEED/BFM calibrations
...
if __name__ == '__main__':
    ...
    calib_Al = Calibration("Al", filename="2018-10-02_16-31-07_Al.txt")
    ...
    with MBERecipe(virtual_server=run_virtual_server, stdby_at_exit=True) as mbe:
        ...
        # TEMPORARY!
        T_Ga = 931  # Set manually since we don't have good RHEED calibration
        T_Ga_Shell = 931  # Set manually since we don't have good RHEED calibration
        T_Al_Shell = 1060  # Set manually since we don't have good RHEED calibration, Al flux seems unstable

        T_Manip_Anneal = 800  # Desired manip temperature
        T_Manip_GaAs = 800  # Desired manip temperature
        T_Manip_Shell = 600  # Desired manip temperature
        ...
        t_growth_almark = thickness_almarkers / growth_rate_nanomembranes  # Assume Al doesn't add much to growth rate
        t_growth_markspac = thickness_markerspacer / growth_rate_nanomembranes
        t_growth_dop = thickness_dop / growth_rate_nanomembranes
        t_growth_barr = thickness_barrier / growth_rate_nanomembranes
        ...
        mbe.set_param("Manip.PV.TSP", T_Manip_Anneal)  # Anneal temperature
        ...
        # Anneal sample
        ##############################################################################
        mbe.wait_to_reach_temp(T_Manip_Anneal)
        mbe.timer_start()
        ...
        ts_print("Going to GaAs growth conditions, setting manip to {}".format(T_Manip_GaAs))
        mbe.set_param("Manip.PV.Rate", 30)
        mbe.set_param("Manip.PV.TSP", T_Manip_GaAs)
        # Wait until growth temperature is reached
        mbe.wait_to_reach_temp(T_Manip_GaAs)

        # Open Ga shutter and start growth
        ts_print("Opening Ga shutter and waiting growth time")
        mbe.shutter("Ga", True)
        mbe.waiting(t_growth_gaas - t_growth_almark - t_growth_barr - t_growth_dop - t_growth_barr - t_growth_almark)
        mbe.shutter("Al", True)
        mbe.waiting(t_growth_almark)
        mbe.shutter("Al", False)
        mbe.waiting(t_growth_barr)
        mbe.shutter("SUSI", True)
        mbe.waiting(t_growth_dop)
        ...
        ts_print("Going to shell growth conditions, setting manip to {}".format(T_Manip_Shell))
        mbe.set_param("Manip.PV.Rate", 30)
        mbe.set_param("Manip.PV.TSP", T_Manip_Shell)
        mbe.set_param("Ga.PV.TSP", T_Ga_Shell)
        mbe.set_param("Al.PV.TSP", T_Al_Shell)

        # Wait for manipulator and cell temperatures
        mbe.wait_to_reach_temp(T_Manip_Shell, error=1)
