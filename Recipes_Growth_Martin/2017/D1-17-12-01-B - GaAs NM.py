...
from os.path import basename as path_to_filename
import sys
...
if __name__ == '__main__':
    ...
    with MBERecipe(virtual_server=run_virtual_server) as mbe:
        ...
        t_growth_gaas = 60 * 60  # 40 minutes (500 nm tall NMs)
        # t_growth_inas = 200  # 200 seconds (4 nm)
        # Si_current = 0  # 0A
        ...
        # p_as_capping = 8E-6
        # as_valve_capping = calib_As.calc_setpoint(p_as_capping)
        # mbe.as_capping(capping_time=30, as_valve=as_valve_capping)
    ...
    if mbe.virtual_server:
        mbe.waiting(60 * 10)  # Wait for cells to cool down to show it in the plot
        ts_print("Plotting the log file")
        mbe.plot_log(path_to_filename(sys.argv[0]).split(" ")[0]+'_VirtualMBEOutput.png')
