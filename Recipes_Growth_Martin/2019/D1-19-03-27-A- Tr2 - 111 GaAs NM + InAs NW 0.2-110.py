...
# Was degassing for like 20h at 400 degrees, we will see if anything grows at all!

# Nanowire growth for Basel on BM4.6 with W markers
# Trying 10 degree lower temps than D1-18-07-11-A, seemed too hot!
# Also using the BM 4.8.1 substate
# Repeat recipe of D1-18-07-12-D to test MBE growth conditions
# Repeat of 19-B this time with infrared camera to check temperature
# Repeat again, this time with manual Ga temperature based on initial RHEED measurements
###########################
Rate_In = 0.2  # A/s
...
if __name__ == '__main__':
    ...
    with MBERecipe(virtual_server=run_virtual_server) as mbe:
        ...
        ###################
        T_Ga = 932  # UGLY TEMPORARY FIX UNTIL WE DO A PROPER RHEED!
        ###################
        T_In = calib_In.calc_setpoint_gr(Rate_In)  # 0.2 A/s growth rate
        # Using Tr2 holder!
        T_Des_Anneal_Manip = 795  # 760  # Desired manip temperature (pyro is broken)
        T_Des_GaAs_Manip = 795  # 750  # Desired manip temperature (pyro is broken)
        T_Des_InAs_Manip = 675  # 640  # Desired manip temperature for InAs growth
