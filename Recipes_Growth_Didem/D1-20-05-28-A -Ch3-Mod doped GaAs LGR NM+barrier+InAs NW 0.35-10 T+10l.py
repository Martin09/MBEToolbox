...
if __name__ == '__main__':
    ...
    with MBERecipe(virtual_server=run_virtual_server) as mbe:
        ...
        T_Ga_Shell = calib_Ga.calc_setpoint_gr(rate_ga)  # Next growthcorrect this
        T_In = calib_In.calc_setpoint_gr(rate_in)  # In temp
        T_In_marker = calib_In.calc_setpoint_gr(rate_in_marker)  # In temp
        T_Anneal_Manip = 770  # Desired manip temperature for Annealing
        T_GaAs_Manip = 770  # Desired manip temperature for GaAs growth
        T_InAs_Manip = 660  # Desired manip temperature for InAs growth
        T_GaAs_Shell_Manip = 480  # Desired manip temperature for GaAs shell
        ...
        thickness_shell = 200  # nm
        t_growth_shell = thickness_shell *10 / rate_ga_shell
		
        growth_rate_nanomembranes = 600.0 / 55.0 / 60.0  # ~800A/30min/60sec

        thickness_gaas = 70  # nm - total GaAs region thickness (for ~50nm wide slits)
        t_growth_gaas = thickness_gaas * 10 / growth_rate_nanomembranes  # Always grow the same thickness of material

        thickness_dop = 5  # nm
        t_growth_dop = thickness_dop*10 / growth_rate_nanomembranes

        thickness_inas = 63  # nm
        t_growth_inas = thickness_inas * 10 / rate_in  # Always grow the same thickness of material

        thickness_gaas_barrier  = 10 #nm
        t_growth_barrier = thickness_gaas_barrier*10 / growth_rate_nanomembranes

        thickness_growth_barrier_low = 5
        t_growth_barrier_low=thickness_gaas_barrier*10 / growth_rate_nanomembranes

        thickness_ingaas_barrier = 5 #nm
        t_growth_ingaas = thickness_ingaas_barrier * 10 / (growth_rate_nanomembranes)
        Si_current = 40  # 40A
        ...
        print("T_Ga_shell: {:.2f} deg C".format(T_Ga_Shell))
        print("T_In_marker: {:.2f} deg C".format(T_In_marker))
        ...
        mbe.set_param("In.PV.TSP", T_In_marker)
        ...
        if run_growth['GaAs_Membrane_Growth']:
            ...
            mbe.shutter("Ga", True)
            mbe.waiting(t_growth_gaas - t_growth_dop -t_growth_ingaas - t_growth_barrier-t_growth_barrier_low)  # Wait Growth Time
            mbe.shutter("Ga",False)

            mbe.set_param("Manip.PV.Rate", 30)
            mbe.set_param("Manip.PV.TSP", T_GaAs_Shell_Manip)
            mbe.wait_to_reach_temp(T_GaAs_Shell_Manip)
            mbe.shutter("Ga", True)
            mbe.shutter("In", True)
            mbe.waiting(t_growth_ingaas)  # Wait Growth Time
            mbe.shutter("In", False)
            mbe.waiting(t_growth_barrier_low)
            mbe.shutter("Ga", False)
            mbe.set_param("Manip.PV.Rate", 30)
            mbe.set_param("Manip.PV.TSP", T_GaAs_Manip)
            mbe.wait_to_reach_temp(T_GaAs_Manip)
            mbe.shutter("Ga", True)
            mbe.waiting(t_growth_barrier)
        ...
        if run_growth['InAs_NW_Growth']:
            ...
            mbe.set_param("Manip.PV.Rate", 30)
            mbe.wait_to_reach_temp(T_In, PID='In', error=1)
        ...
        if run_growth['GaAs_passivation_layer']:
            ...
            mbe.set_param("In.PV.TSP", T_In_marker)
            mbe.wait_to_reach_temp(T_In_marker, error=1)
            ...
            mbe.shutter("Ga", True)
            mbe.waiting(t_growth_shell*0.25)  # Wait Growth Time
            mbe.shutter("In", True)
            mbe.waiting(t_growth_ingaas)  # Wait Growth Time
            mbe.shutter("In", False)
            mbe.waiting(t_growth_shell*0.75)  # Wait Growth Time
            mbe.shutter("Ga", False)
