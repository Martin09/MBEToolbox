...
# Chip ID: 0000456728-- with metal AMs.
...
if __name__ == '__main__':
    ...
    with MBERecipe(virtual_server=run_virtual_server) as mbe:
        ...
        T_InAs_Manip = 610  # Desired manip temperature for InAs growth
        T_GaAs_Shell_Manip = 440  # Desired manip temperature for GaAs
    ...
    if run_growth['GaAs_Membrane_Growth']:
        ...
        file.write("T manipulator during GaAs: {:.0f} deg C\n".format(T_GaAs_Manip))
        file.write("rate_ga_for gaas: {:.1f} A/s\n".format(rate_ga))
    ...
    if run_growth['InAs_NW_Growth']:
        ...
        file.write("T manipulator during InAs: {:.0f} deg C\n".format(T_InAs_Manip))
        file.write("rate_in_for inas: {:.1f} A/s\n".format(rate_in))
    ...
    if run_growth['GaAs_passivation_layer']:
        ...
        file.write("T_Ga during GaAs passivation layer: {:.0f} deg C \n".format(T_Ga_Shell))
        file.write("p_as_gaas: {:.2e} Torr\n".format(p_as_shell))
