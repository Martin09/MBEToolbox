...
if __name__ == '__main__':
    ...
    with MBERecipe(virtual_server=run_virtual_server) as mbe:
        ...
        p_as_annealing = 1.0E-5
        as_valve_annealing = calib_As.calc_setpoint(p_as_annealing)
        ...
        t_anneal = 2 * 60  # 2 minutes
        thickness_gaas = 75  # nm
        ...
        print("p_as_annealing: {:.2e} Torr".format(p_as_annealing))
        #print("p_as_shell: {:.2e} Torr".format(p_as_shell))
        print("as_valve_gaas: {:.2f}%".format(as_valve_gaas))
        print("as_valve_annealing: {:.2f}%".format(as_valve_annealing))
        ...
        mbe.set_param("AsCracker.Valve.OP", as_valve_annealing)
        mbe.shutter("As", True)
        mbe.waiting(60 * 3)  # Wait 3min
        ts_print("Checking pressure")
        if float(mbe.get_param("MBE.P")) < 5e-9:  # Make sure As valve opened properly
    ...
    if run_growth['Annealing']:
        ...
        file.write("p_as_nnealing: {:.2e} Torr\n".format(p_as_annealing))
        file.write("as_valve_annealing: {:.2f}%\n".format(as_valve_annealing))
