...
if __name__ == '__main__':
    ...
    with MBERecipe(virtual_server=run_virtual_server) as mbe:
        ...
        T_Des_Shell_Manip = 580  # Desired manip temperature (since not using pyro)
        ...
        thickness_alas_shell = 50  # nm
        GR_alas_shell = rate_al_shell / 4.  # Shell grows at 1/4 rate of a 2D layer (for vert. NWs)
        t_growth_alas = thickness_alas_shell * 10. / GR_alas_shell  # sec

        thickness_gaas_shell = 10  # nm, normally 5 nm but we want to be safe and covered on the bottom of the NWs
        ...
        # AlGaAs Shell
        ##############################################################################
        # Set AlGaAs growth conditions
        ts_print("Going to AlGaAs growth conditions, setting manip to {}".format(T_Des_Shell_Manip))
        mbe.set_param("Manip.PV.Rate", 30)
