...
Accidentally modified this recipe, need to fix it if I ever want to use it again!
...
if __name__ == '__main__':
    ...
    with MBERecipe(virtual_server=run_virtual_server, scriptname=os.path.basename(__file__)) as mbe:
        ...
        T_Ga = calib_Ga.calc_setpoint_gr(desired_gr=0.8)  # 2.4E-7 originally, but Ga flux seems to be low (2.4*0.7)
        ...
        p_as_gaas = 2.8E-6  # Desired As pressure for GaAs growth
        ...
        # as_valve_gaas = mbe.converge_with_bfm(p_as_gaas, calib_As, withdraw_bfm=False)  # Takes about 5 min

        # # Converge with BFM to desired nucleation As pressure, save the correct As opening for use later
        # mbe.ts_print("Converging with BFM for As pressure in nucleation, saving it for later")
        # as_valve_nucl = mbe.converge_with_bfm(p_as_inas, calib_As)  # Takes about 5 min
        ...
        mbe.ts_print("Ramping up Ga source")
        ...
        mbe.shutter("Ga", True)
        mbe.waiting(t_nucleate)     # Wait Nucleation Time
