...
if __name__ == '__main__':
    ...
    with MBERecipe(virtual_server=run_virtual_server) as mbe:
        ...
        T_Des_GaAs2_Manip = T_Des_InAs_Manip  # Desired manip temperature for GaAs spacer growth	
        p_as_gaas = 4E-6  # Desired As pressure for GaAs NM growth
        p_as_inas = 8E-6  # Desired As pressure for InAs NW growth
        p_as_gaas2 = 1E-5  # Desired As pressure for GaAs spacer growth
        ...
        t_growth_gaas2 = 30  # 30 seconds (3nm)
        Si_current = 40  # 40A
        ...
        # Go to GaAs spacer growth temp
        # ts_print("Going to InAs growth conditions, setting manip to {}".format(T_Des_InAs + mbe.manip_offset))
        ts_print("Going to GaAs spacer growth conditions, setting manip to {}".format(T_Des_GaAs2_Manip))
        mbe.set_param("Manip.PV.Rate", 30)
        # mbe.set_param("Manip.PV.TSP", T_Des_InAs + mbe.manip_offset)
        # mbe.wait_to_reach_temp(T_Des_InAs + mbe.manip_offset, error=1)
        mbe.set_param("Manip.PV.TSP", T_Des_GaAs2_Manip)
        mbe.wait_to_reach_temp(T_Des_GaAs2_Manip, error=1)
		
        # Set As flux for GaAs spacer
        mbe.set_param("AsCracker.Valve.OP", as_valve_gaas2)

        mbe.waiting(60)  # Wait for As valve to be set and stabilize
