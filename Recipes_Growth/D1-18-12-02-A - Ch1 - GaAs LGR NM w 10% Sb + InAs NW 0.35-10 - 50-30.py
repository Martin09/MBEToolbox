...
# 11-05-A - Added Sb flux, same as As pressure. CAREFUL GA TEMP IS SET TOO LOW BY CALIBRATION FILE!
# 11-07-A - Manually set Ga temp to 915 and In temps to 795.7, new BFM reads totally different pressures!
# 11-08-A - Using the old calibration files, don't trust new BFM before we do a (full) RHEED calibration.
# 11-09-D - Reduced growth time by factor 2 + added Sb tank heat-up/cool-down commands
# 11-30-E - Repeat of 11-09-D which failed due to Sb valve motor fault
# 11-30-E - Repeat of 11-09-D which failed due to Sb valve motor fault (faults at 60% opening)
###########################
rate_ga = 0.3  # A/s
ftr_gaas = 80  # five three ratio
rate_in = 0.35  # A/s
ftr_inas = 10  # five three ratio
sb_fraction = 0.15  # (add a bit more Sb to offset the higher pressures read in the new BFM)
###########################
...
if __name__ == '__main__':
    ...
    calib_Sb = Calibration("Sb", filename="2018-11-06_09-15-31_Sb.txt")
    ...
    p_sb_gaas = sb_fraction * p_as_gaas
    sb_valve_gaas = calib_Sb.calc_setpoint(p_sb_gaas)
    ...
    p_sb_inas = sb_fraction * p_as_inas
    sb_valve_inas = calib_Sb.calc_setpoint(p_sb_inas)
    ...
    print("Sb pressures {:.2E}, {:.2E}".format(p_sb_gaas, p_sb_inas))
    print("Sb valve positions {:.0f}, {:.0f}".format(sb_valve_gaas, sb_valve_inas))
    ...
    with MBERecipe(virtual_server=run_virtual_server) as mbe:
        ...
        mbe.set_param("SbCracker.Valve.OP", sb_valve_gaas)
        mbe.shutter("Sb", True)
        ...
        mbe.set_param("SbCracker.Valve.OP", sb_valve_inas)
        mbe.shutter("Sb", True)
