...
if __name__ == '__main__':
    ...
    with MBERecipe(virtual_server=run_virtual_server) as mbe:
        ...
        T_Des_Anneal = 610  # Desired pyrometer temperature for anneal
        T_Des_GaAs = 630  # Desired pyrometer temperature for GaAs growth
        T_Des_Anneal_Manip = 750  # Desired manip temperature (if pyro is broken)
        T_Des_GaAs_Manip = 750  # Desired manip temperature (if pyro is broken)
        ...
        t_gapredep = 10 * 60  # 10 minutes @ 1 A/s, same as Wonjong
        ...
        # Close As, open Ga for Ga predeposition
        ts_print("Closing As, opening Ga for Ga predep")
        mbe.shutter("As", False)
        mbe.shutter("Ga", True)
        mbe.waiting(t_gapredep)  # Wait Ga predeposition time
        mbe.shutter("As", True)
