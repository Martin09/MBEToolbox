...
This is a first attempt to grow InSb NWs on top of GaAs NMs
Sb pressure and growth temperature is unknown for now.
...
if __name__ == '__main__':
    ...
    with MBERecipe(virtual_server=run_virtual_server) as mbe:
        ...
        T_Des_InSb_Manip = 515  # Desired manip temperature for GaSb growth
        p_as_gaas = 4E-6  # Desired As pressure for GaAs growth
        p_sb_insb = 1E-5  # Desired Sb pressure for InSb growth
        # NOT SURE ABOUT THIS PRESSURE RIGHT NOW!!
        as_valve_gaas = calib_As.calc_setpoint(p_as_gaas)
        sb_valve_insb = calib_Sb.calc_setpoint(p_sb_insb)
        ...
        # Go to InSb growth temp
        # ts_print("Going to InAs growth conditions, setting manip to {}".format(T_Des_InAs + mbe.manip_offset))
        ts_print("Going to InSb growth conditions, setting manip to {}".format(T_Des_InSb_Manip))
        mbe.set_param("Manip.PV.Rate", 30)
        # mbe.set_param("Manip.PV.TSP", T_Des_InSb + mbe.manip_offset)
        # mbe.wait_to_reach_temp(T_Des_InSb + mbe.manip_offset, error=1)
        mbe.set_param("Manip.PV.TSP", T_Des_InSb_Manip)
        mbe.wait_to_reach_temp(T_Des_InSb_Manip, error=1)

        # Close As and set Sb flux, let it open during final temperature convergence
        mbe.set_param("SbCracker.Valve.OP", sb_valve_insb) # THIS IS NOT GOING TO WORK!
        ...
        mbe.shutter("In", True)
        mbe.waiting(t_growth_insb)  # Wait Growth Time
