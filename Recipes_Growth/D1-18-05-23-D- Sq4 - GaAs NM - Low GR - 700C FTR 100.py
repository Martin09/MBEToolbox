...
###########################
gaas_growth_temp = 700  # about 580 pyro temperature
five_three_ratio = 80
###########################
...
if __name__ == '__main__':
    ...
    with MBERecipe(virtual_server=run_virtual_server) as mbe:
        ...
        Rate_Ga = 0.3
        # Rate_In = 0.3
        T_Ga = calib_Ga.calc_setpoint_gr(0.3)  # 0.3 A/s growth rate
        # T_In = calib_In.calc_setpoint_gr(0.3)  # 0.3 A/s growth rate
        ...
        T_Des_GaAs_Manip = gaas_growth_temp  # Desired manip temperature (pyro is broken)
        # T_Des_InAs_Manip = 640  # Desired manip temperature for InAs growth
        p_as_gaas = calib_Ga.calc_p_arsenic(Rate_Ga, five_three_ratio)  # Desired As pressure for GaAs growth
        # p_as_inas = calib_In.calc_p_arsenic(Rate_In, five_three_ratio)  # Desired As pressure for InAs growth
        as_valve_anneal = calib_As.calc_setpoint(4E-6)
