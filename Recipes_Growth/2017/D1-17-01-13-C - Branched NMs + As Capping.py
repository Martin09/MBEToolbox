...
if __name__ == '__main__':
    ...
    calib_In = Calibration("In", filename='2016-02-24_19-47-46_In.txt')
    calib_Ga = Calibration("Ga", filename='2016-11-16_14-30-51_Ga.txt')
    calib_As = Calibration("As", filename='2017-01-06_12-21-29_As.txt')
    ...
    with MBERecipe(virtual_server=run_virtual_server) as mbe:
        ...
        mbe.waiting(60)  # wait 60a before starting the recipe

        ts_print("Setting variables")
        # Define growth parameters
        T_Ga = calib_Ga.calc_setpoint(2.4E-7)
        T_In = calib_In.calc_setpoint(1E-7)  # roughly 0.2A/s
        ...
        As_Valve = calib_As.calc_setpoint(3E-6) + 5  # NOTE: INCREASED ARSENIC SETPOINT BY 5% BECAUSE OF LOW AS PRESSURE
        ...
        mbe.waiting(60 * 25)  # Wait 25min (total about 30min)
        ...
        # Don't set standby because it would heat manipulator to 200 and uncap the sample...
