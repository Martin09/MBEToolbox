...
if __name__ == '__main__':
    ...
    with MBERecipe(virtual_server=run_virtual_server) as mbe:
        ...
        # Start InAs Growth
        ts_print("Opening In + Si shutter and waiting growth time")
        mbe.shutter("In", True)		
        mbe.shutter("SUSI", True)		
        mbe.waiting(int(t_growth_inas*0.5))  # Wait 50% of Growth Time
        ts_print("Closing Si shutter")		
        mbe.shutter("SUSI", False)
        mbe.waiting(int(t_growth_inas*0.50))  # Wait last 50% of Growth Time
