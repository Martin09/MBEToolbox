...
# This time using a low growth rate!
###########################
thickness_almarker1 = 5
...
thickness_gaas_shell = 200
rate_ga = 0.3  # A/s - Will calculate cell temp from this based on latest RHEED/BFM calibrations
rate_al = 0.1  # A/s - Will calculate cell temp from this based on latest RHEED/BFM calibrations
...
if __name__ == '__main__':
    ...
    with MBERecipe(virtual_server=run_virtual_server) as mbe:
        ...
        T_Al = calib_Al.calc_setpoint_gr(rate_al)
        ...
        growth_rate_nanomembranes = 100.0 / 30.0 / 60.0  # ~100nm/30min/60sec
        ...
        Si_current = 28  # Slightly lower this due to lower growth rate!!!!
        ...
        mbe.shutter("Ga", True)
        mbe.waiting(
            t_growth_gaas - t_growth_almarker - t_growth_markerspacer - t_growth_dop - t_growth_barrier)
        ...
        mbe.wait_to_reach_temp(PID='Ga', error=1)
        mbe.wait_to_reach_temp(PID='Al', error=1)
