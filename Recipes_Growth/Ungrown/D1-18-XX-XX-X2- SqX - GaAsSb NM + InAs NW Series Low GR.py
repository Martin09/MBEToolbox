...
#TODO: Add antimony to the GaAs growth!
...
Rate_In = 0.3  # A/s
Rate_Ga = 0.3  # A/s
P_As_InAs = 1.0E-5  # Torr
P_As_GaAs = 4.0E-6  # Torr
###########################
...
if __name__ == '__main__':
    ...
    with MBERecipe(virtual_server=run_virtual_server) as mbe:
        ...
        T_Des_GaAs = 500  # Desired pyrometer temperature for GaAs growth
        T_Des_InAs = 500  # Desired pyrometer temperature for InAs growth
        T_Des_Anneal_Manip = T_Des_Anneal + mbe.manip_offset  # Desired manip temperature (if pyro is broken)
        T_Des_GaAs_Manip = T_Des_GaAs + mbe.manip_offset  # Desired manip temperature (if pyro is broken)
        T_Des_InAs_Manip = T_Des_InAs + mbe.manip_offset  # Desired manip temperature for InAs growth (if pyro is broken)
        p_as_gaas = P_As_GaAs  # Desired As pressure for GaAs growth
        p_as_inas = P_As_InAs  # Desired As pressure for InAs growth
        ...
        t_growth_gaas = thickness_gaas * 10 / Rate_Ga
        thickness_inas = 10  # nm
        t_growth_inas = thickness_inas * 10 / Rate_In
