"""
Example of a growth recipe for nanomembranes with InGaAs nanowires on top.
...
if __name__ == '__main__':
    ...
    with MBERecipe(virtual_server=run_virtual_server) as mbe:
        ...
        gr_ingas = 0.2  # 0.2 A/s growth rate
        conc_in = 0.2  # Indium concentration
        gr_in = gr_ingas * conc_in
        gr_ga2 = gr_ingas - gr_in

        # Define growth parameters
        T_Ga = calib_Ga.calc_setpoint_gr(1.0)  # 1.0 A/s growth rate
        T_In = calib_In.calc_setpoint_gr(gr_in)  # growth rate
        T_Ga2 = calib_Ga.calc_setpoint_gr(gr_ga2)  # growth rate
        # T_Des_Anneal = 600  # Desired pyrometer temperature for anneal
        # T_Des_GaAs = 630  # Desired pyrometer temperature for GaAs growth
        # T_Des_InGaAs = 535  # Desired pyrometer temperature for InAs growth
        ...
        T_Des_InGaAs_Manip = 650  # Desired manip temperature for InAs growth
        ...
        t_growth_ingaas = 200  # 200 seconds (4nm)
        # Si_current = 0  # 0A
        ...
        # Go to InGaAs growth temp
        # ts_print("Going to InGaAs growth conditions, setting manip to {}".format(T_Des_InGaAs + mbe.manip_offset))
        ts_print("Going to InGaAs growth conditions, setting manip to {}".format(T_Des_InGaAs_Manip))
        mbe.set_param("Manip.PV.Rate", 30)
        # mbe.set_param("Manip.PV.TSP", T_Des_InGaAs + mbe.manip_offset)
        # mbe.wait_to_reach_temp(T_Des_InGaAs + mbe.manip_offset, error=1)
        mbe.set_param("Manip.PV.TSP", T_Des_InGaAs_Manip)
        mbe.set_param("Ga.PV.TSP", T_Ga2)
        mbe.set_param("In.PV.TSP", T_In)
        mbe.wait_to_reach_temp(T_Des_InGaAs_Manip, error=1)
        ...
        # mbe.converge_to_temp(T_Des_InGaAs)  # Takes about 4min

        # Start InAs Growth
        ts_print("Opening In/Ga shutters and waiting growth time")
        mbe.shutter("In", True)
        mbe.shutter("Ga", True)
