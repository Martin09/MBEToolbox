...
# 12-17-D - GaAs NMs with 5E-7 Sb
# 12-20-C - Lowered both anneal and growth temp down to 750 manip temp, otherwise same as 12-17-D
###########################
rate_ga = 0.3  # A/s
...
if __name__ == '__main__':
    ...
    calib_Sb = Calibration("Sb", bfm_correction=conv_func_As, filename="2018-12-11_10-25-33_T450_Sb.txt")
    ...
    T_In = calib_In.calc_setpoint_gr(rate_in)
    T_Sb = 450
    ...
    with MBERecipe(virtual_server=run_virtual_server) as mbe:
        ...
        # Ramp up Sb tank first
        mbe.set_param("Sb.PV.Rate", 5)
        mbe.set_param("Sb.OP.Rate", 0)
        mbe.set_param("Sb.PV.TSP", T_Sb)
        mbe.wait_to_reach_temp(temp=T_Sb, PID="Sb")
        mbe.waiting(10 * 60)  # Wait 10min to stabilize temp
        ...
        mbe.waiting(60 * 1)  # Wait 1min to stabilize flux
